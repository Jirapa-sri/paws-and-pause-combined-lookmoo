...
`.trim();
   ...
   * WebAudio-first via host PawsEnsureAudio (same unlock as footsteps / stadium).
   * File buffers from js/buildings/arcade/audio/; synth fallback if missing.
   * No arcade-only Enable Sound UI — plays by default after walking unlocks audio.
   */
  ...
  // Match town/building ambience + stadium hub (walk SFX ~0.05; beds ~0.02)
  const BGM_VOLUME = 0.22;
  const SFX_VOLUME = 0.42;
  const SFX_LOOP_VOLUME = 0.38;
  ...
  const ArcadeAudio = (function () {
    ...
    function withAc(fn) {
      ...
      safe(() => {
        ...
        if (p && typeof p.then === "function") {
          p.then(() => { if (ac.state === "running") safe(() => fn(ac)); }).catch(() => {});
        }
      });
    ...
    function isBgmPlaying() {
      return !!(bgmSource || bgmSynthTimer);
    }

    function clearSoundPrompt() {
      const btn = document.getElementById("arcade-enable-sound");
      if (btn) btn.remove();
    }
    ...
    function loadBuffer(key, cb) {
      const file = AUDIO_FILES[key];
      if (!file || missing[file]) { if (cb) cb(null); return; }
      if (bufferCache[file]) { if (cb) cb(bufferCache[file]); return; }
      const ac = getAc();
      if (!ac) { if (cb) cb(null); return; }
      safe(() => {
        fetch(AUDIO_DIR + file)
          ...
          .then((arr) => ac.decodeAudioData(arr.slice(0)))
          .then((buf) => {
    ...
    function preloadAll() {
      ...
      preloadStarted = true;
      Object.keys(AUDIO_FILES).forEach((key) => loadBuffer(key));
    }
    ...
    function playBuffer(key, opts) {
      ...
      return !!withAc((ac) => {
        ...
        src.buffer = buf;
        src.loop = !!opts.loop;
        gain.gain.value = opts.volume != null ? opts.volume : SFX_VOLUME;
        src.connect(gain);
        gain.connect(ac.destination);
        src.start();
        if (opts.loop && opts.onLoopNodes) opts.onLoopNodes(src, gain);
      });
    }

    const synth = {
      reelSpinClick() { beep(220 + Math.random() * 50, 0.05, "square", 0.1); noise(0.03, 0.05); },
      jackpotFanfare() { [523, 659, 784, 1047, 1319].forEach((f, i) => beep(f, 0.16, "square", 0.16, i * 0.09)); },
      coinShower() { for (let i = 0; i < 8; i++) beep(880 + i * 40, 0.07, "triangle", 0.14, 0.05 + i * 0.05); },
      dingSuccess() { beep(880, 0.09, "triangle", 0.16); beep(1175, 0.14, "triangle", 0.14, 0.07); },
      buzzFail() { beep(140, 0.2, "sawtooth", 0.14); beep(100, 0.16, "sawtooth", 0.12, 0.08); },
      jump() { beep(320, 0.1, "square", 0.13); beep(560, 0.12, "square", 0.12, 0.04); },
      coin() { beep(988, 0.07, "square", 0.14); beep(1319, 0.1, "square", 0.12, 0.04); },
      stomp() { beep(180, 0.08, "square", 0.14); noise(0.06, 0.07); },
      stageClear() { [523, 659, 784, 1047].forEach((f, i) => beep(f, 0.16, "square", 0.16, i * 0.1)); },
      raceStart() { beep(440, 0.12, "square", 0.16); beep(440, 0.12, "square", 0.16, 0.35); beep(700, 0.22, "square", 0.18, 0.7); },
      footstepFast() { noise(0.04, 0.05); beep(240 + Math.random() * 40, 0.04, "square", 0.08); },
      victoryCheer() { [523, 659, 784, 988, 1175].forEach((f, i) => beep(f, 0.14, "triangle", 0.16, i * 0.08)); },
      power() { [440, 554, 659, 880].forEach((f, i) => beep(f, 0.09, "triangle", 0.14, i * 0.06)); },
      block() { beep(300, 0.06, "square", 0.12); beep(500, 0.07, "triangle", 0.1, 0.04); },
      bark() { beep(190, 0.08, "sawtooth", 0.14); beep(150, 0.1, "sawtooth", 0.12, 0.07); },
      place() { [392, 523, 659].forEach((f, i) => beep(f, 0.11, "triangle", 0.14, i * 0.07)); },
    };

    function playSfx(key, fallbackName, volume) {
      bindHostSoundButton();
      clearSoundPrompt();
      if (playBuffer(key, { loop: false, volume: volume != null ? volume : SFX_VOLUME })) return;
    ...
    function stopBgmNodes() {
      safe(() => { if (bgmSource) bgmSource.stop(0); });
      safe(() => { if (bgmGain) bgmGain.disconnect(); });
      bgmSource = null;
      bgmGain = null;
    }
    ...
    function startSynthBgm() {
      ...
      const tick = () => {
        ...
        beep(f, 0.28, "sawtooth", 0.08);
        beep(f * 2, 0.18, "square", 0.05, 0.02);
        if (step % 4 === 0) beep(98, 0.35, "triangle", 0.08);
        step++;
      };
      if (!withAc(() => {})) return false;
      pauseTownBgm();
      tick();
    ...
    function startFileBgm(buf) {
      if (!buf || !wantsBgm) return false;
      return !!withAc((ac) => {
        stopSynthBgm();
        stopBgmNodes();
        pauseTownBgm();
    ...
    function startBgmNow() {
      ...
      clearSoundPrompt();
      pauseTownBgm();
      const cached = bufferCache[AUDIO_FILES.bgm];
      if (cached && !missing[AUDIO_FILES.bgm]) {
        if (startFileBgm(cached)) return;
      }
      if (!isBgmPlaying()) startSynthBgm();
      loadBuffer("bgm", (buf) => {
        if (!buf || !wantsBgm) return;
        startFileBgm(buf);
      });
    }
    ...
    function resumeTownBgm() {
      safe(() => {
        ...
        if (window.PawsStadiumAudio && window.PawsStadiumAudio.isBgmPlaying && window.PawsStadiumAudio.isBgmPlaying()) return;
    ...
    function playBgm() {
      ...
      preloadAll();
      unlock();
      startBgmNow(); // pauseTownBgm runs only once audio actually starts
      if (!isBgmPlaying()) {
        let tries = 0;
        const iv = setInterval(() => {
          tries++;
          if (!wantsBgm || isBgmPlaying() || tries > 24) {
            clearInterval(iv);
            return;
          }
          unlock();
    ...
    function stopBgm(immediate) {
      ...
      const fade = setInterval(() => {
        v = Math.max(0, v - 0.05);
        safe(() => { gain.gain.value = v; });
        if (v <= 0.01) {
          clearInterval(fade);
          safe(() => { if (src) src.stop(0); });
          safe(() => { gain.disconnect(); });
          resumeTownBgm();
        }
      }, 40);
    }
    ...
    function startReelSpin() {
      stopReelSpin();
      ensureArmed();
      const started = playBuffer("reelSpin", {
        loop: true,
        volume: SFX_LOOP_VOLUME,
        onLoopNodes(src, gain) {
          spinSource = src;
          spinGain = gain;
        },
      });
      if (!started) {
        ...
        loadBuffer("reelSpin", (buf) => {
          if (!buf || !spinSynthTimer) return;
    ...
    function stopReelSpin() {
      ...
      safe(() => { if (spinSource) spinSource.stop(0); });
      safe(() => { if (spinGain) spinGain.disconnect(); });
      spinSource = null;
      spinGain = null;
    }
    ...
    return {
      ...
      jackpotWin() {
        stopReelSpin();
        if (!playBuffer("jackpotFanfare", { volume: 0.5 })) synth.jackpotFanfare();
        setTimeout(() => {
          if (!playBuffer("coinShower", { volume: 0.45 })) synth.coinShower();
        }, 280);
      },
      smallWin() { stopReelSpin(); playSfx("dingSuccess", "dingSuccess", SFX_VOLUME); },
      lose() { stopReelSpin(); playSfx("buzzFail", "buzzFail", SFX_VOLUME); },
      jump() { playSfx("jump", "jump", 0.4); },
      coin() { playSfx("coin", "coin", SFX_VOLUME); },
      stomp() { playSfx("stomp", "stomp", SFX_VOLUME); },
      stageClear() { playSfx("stageClear", "stageClear", 0.5); },
      raceStart() { playSfx("raceStart", "raceStart", 0.48); },
      tap() { playSfx("footstepFast", "footstepFast", 0.38); },
      victoryCheer() { playSfx("victoryCheer", "victoryCheer", 0.5); },
      power() { synth.power(); },
      block() { synth.block(); },
      bark() { synth.bark(); },
      place() { synth.place(); },
      win() { playSfx("stageClear", "stageClear", 0.5); },
      hit() { beep(880, 0.06, "triangle", 0.14); },
      spin() { synth.reelSpinClick(); },
    };
  })();
  ...
  // Light gesture unlock while inside arcade (throttled — avoid freezing input)
  let lastArcadeArm = 0;
  ["pointerdown", "keydown", "mousedown", "touchstart"].forEach((ev) => {
    window.addEventListener(ev, () => {
      if (!arcadeActive && !modalOpen) return;
      const now = performance.now();
      if (now - lastArcadeArm < 250) return;
      lastArcadeArm = now;
      ArcadeAudio.armFromGesture();
    }, true);
  });
  ...
  function onArcadeEnter(api) {
    ...
    hideHostBanner();
    // Start BGM by default (host walking / HUD sound already unlocks audio)
    ArcadeAudio.playBgm();
  }
  ...
  function openPixelQuest(api) {
    if (api && api.state) lastApi = api;
    const state = lastApi && lastApi.state;
    if (!state) return;
