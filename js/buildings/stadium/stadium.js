...
`.trim();
   ...
   */
  // Match arcade BGM (0.22 HTMLAudio) + town beds; walk SFX ~0.05
  const HUB_BGM_VOLUME = 0.4;
  const RACE_BGM_VOLUME = 0.34; // present under gameplay, not whisper-quiet
  ...
  const StadiumAudio = (function () {
    ...
    function startHubNow() {
      ...
      const tick = () => {
        ...
        if (step % 2 === 0) beep(bass[(step / 2) % bass.length], 0.28, "triangle", 0.07 * mul);
        if (step % 4 === 0) beep(pad[(step / 4) % pad.length], 0.4, "sine", 0.05 * mul, 0.02);
        if (step % 8 === 0) noise(0.04, 0.02 * mul);
        step++;
      };
      const ok = withAc(() => {});
      if (!ok) return false;
    ...
    function startRaceNow() {
      ...
      const tick = () => {
        ...
        if (step % 2 === 0) beep(bass[(step / 2) % bass.length], 0.14, "sine", 0.07 * mul);
        // Airy pad, sparse
        if (step % 4 === 0) {
          const p = pad[(step / 4) % pad.length];
          beep(p, 0.45, "sine", 0.045 * mul);
          beep(p * 1.5, 0.4, "triangle", 0.028 * mul, 0.03);
        }
        // Very light hat once per bar
        if (step % 8 === 4) noise(0.03, 0.028 * mul);
        step++;
      };
      const ok = withAc(() => {});
      if (!ok) return false;
   ...
   * ================================================================ */
  async function openGrandPrix(api) {
