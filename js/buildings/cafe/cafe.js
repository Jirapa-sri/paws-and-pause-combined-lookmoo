...
(function () {
  ...
  function ensureMsgEl() {
    ...
    el.appendChild(card);
    cafeStageHost().appendChild(el);
    return el;
  }

  function ensureTimerEl() {
    let el = document.getElementById("cafe-chase-timer");
    if (el) return el;
    el = document.createElement("div");
    el.id = "cafe-chase-timer";
    el.style.cssText = [
      "position:absolute", "left:50%", "top:18px", "transform:translateX(-50%)",
      "width:min(280px,78%)", "display:none", "pointer-events:none", "z-index:45",
      "font-family:ui-rounded, system-ui, sans-serif", "color:#FBF0DE",
      "background:rgba(58,44,34,0.86)", "border:2px solid #C4785A",
      "border-radius:14px", "padding:0.45rem 0.7rem 0.55rem",
      "box-shadow:0 8px 22px rgba(42,33,24,0.28)",
    ].join(";");
    el.innerHTML = [
      '<div style="display:flex;justify-content:space-between;align-items:baseline;gap:0.6rem;margin-bottom:0.35rem;">',
      '<span style="font-weight:800;font-size:0.82rem;letter-spacing:0.02em;">Catch timer</span>',
      '<span id="cafe-chase-timer-sec" style="font-weight:800;font-size:1rem;font-variant-numeric:tabular-nums;">15s</span>',
      "</div>",
      '<div style="height:10px;border-radius:999px;background:rgba(251,240,222,0.18);overflow:hidden;">',
      '<div id="cafe-chase-timer-fill" style="height:100%;width:100%;border-radius:999px;background:linear-gradient(90deg,#C4785A,#E8A33D);transition:width 0.08s linear;"></div>',
      "</div>",
    ].join("");
    cafeStageHost().appendChild(el);
    return el;
  ...
  function syncChaseTimerDom() {
    const el = ensureTimerEl();
    const active = !!(chase && chase.phase === "chase" && !chase.resolved);
    if (!active) {
      el.style.display = "none";
      return;
    }
    const sec = Math.ceil(Math.max(0, chase.timeLeft));
    const frac = Math.max(0, Math.min(1, chase.timeLeft / CHASE_SECONDS));
    el.style.display = "block";
    const secEl = document.getElementById("cafe-chase-timer-sec");
    const fill = document.getElementById("cafe-chase-timer-fill");
    if (secEl) {
      secEl.textContent = sec + "s";
      secEl.style.color = sec <= 5 ? "#F0C36A" : "#FBF0DE";
    }
    if (fill) {
      fill.style.width = (frac * 100) + "%";
      fill.style.background = sec <= 5
        ? "linear-gradient(90deg,#C0483E,#F0C36A)"
        : "linear-gradient(90deg,#C4785A,#E8A33D)";
    }
  }
  ...
  function onCafeExit() {
    ...
    const el = document.getElementById("cafe-chase-msg");
    if (el) el.style.display = "none";
    const timer = document.getElementById("cafe-chase-timer");
  ...
  function drawChaseHud(ctx, W) {
    // Canvas fallback (DOM timer is primary — sits above the wooden frame)
    if (!chase || chase.phase !== "chase" || chase.resolved) return;
    if (document.getElementById("cafe-chase-timer")) return;
    const sec = Math.ceil(Math.max(0, chase.timeLeft));
    const frac = Math.max(0, Math.min(1, chase.timeLeft / CHASE_SECONDS));
    ctx.save();
    const bw = 220, bh = 38;
    const bx = (W || 900) / 2 - bw / 2;
    const by = 18; // below wooden frame (~14px)
    ctx.fillStyle = "rgba(58,44,34,0.78)";
    roundRectPath(ctx, bx, by, bw, bh, 12); ctx.fill();
    ctx.fillStyle = "rgba(251,240,222,0.2)";
    roundRectPath(ctx, bx + 12, by + 24, bw - 24, 8, 4); ctx.fill();
    ctx.fillStyle = sec <= 5 ? "#F0C36A" : "#E8A33D";
    roundRectPath(ctx, bx + 12, by + 24, (bw - 24) * frac, 8, 4); ctx.fill();
    ctx.fillStyle = sec <= 5 ? "#F0C36A" : "#FBF0DE";
    ctx.font = "700 12px ui-rounded, sans-serif";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText("Catch timer · " + sec + "s", bx + bw / 2, by + 13);
    ctx.restore();
  }
  ...
  function tickCafe(ctx, api, t, cx, cy) {
    ...
    // Keep café logic on the same position the host draws (player sprite)
    if (api.player) {
      mirror.x = api.player.x;
      mirror.y = api.player.y;
    } else if (!menuOpen) {
    ...
    if (!menuOpen) updateChase(dt, HOUSE, state);
